...
export class AiService {
  ...
  constructor(
    ...
    const geminiKey =
      ...
      this.config.get<string>('GEMINI_KEY');
    this.geminiModel =
      this.config.get<string>('GEMINI_MODEL') || 'models/gemini-2.5-flash';
  ...
  async chatWithPatient(
    ...
    const contextText = patientContext
      ? `Contexte: ${JSON.stringify(patientContext)}\n`
      : '';
  ...
  private async collectData(dateStr?: string) {
    ...
    const [vitalsToday, symptomsToday, vitalsYesterday, symptomsYesterday] =
      await Promise.all([
        this.vitalModel
          .find({
        ...
        this.symptomModel
          .find({
            patientId: { $in: patientIds },
            reportedAt: { $gte: yesterdayStart, $lte: yesterdayEnd },
          })
          .lean(),
      ]);

    const buildSet = (docs: any[], field: string) =>
      new Set(docs.map((d) => d[field]?.toString()));
    ...
    const okToday = (patients as any[]).filter(
      (p) =>
        vitalTodaySet.has(p._id.toString()) &&
        symptomTodaySet.has(p._id.toString()),
    ).length;
    const okYesterday = (patients as any[]).filter(
      (p) =>
        vitalYestSet.has(p._id.toString()) &&
        symptomYestSet.has(p._id.toString()),
    ).length;

    const complianceToday = totalPatients
      ? Math.round((okToday / totalPatients) * 100)
      : 0;
    const complianceYesterday = totalPatients
      ? Math.round((okYesterday / totalPatients) * 100)
      : 0;

    // ── Reminder Analytics — même période (aujourd'hui) ──────────
    const sentToday = (remindersToday as any[]).filter(
      (r) => r.status === 'sent',
    ).length;
    const answeredToday = (remindersToday as any[]).filter(
      (r) => r.status === 'answered',
    ).length;
    const missedToday = (remindersToday as any[]).filter(
      (r) => r.status === 'missed',
    ).length;
    const pendingToday = (remindersToday as any[]).filter(
      (r) => r.status === 'pending',
    ).length;
    const sentReminders = sentToday;
    const responseRate = sentToday
      ? Math.round((answeredToday / sentToday) * 100)
      : 0;

    const alerts: string[] = [];
    if (complianceToday === 0) alerts.push("Aucun patient actif aujourd'hui");
    if (responseRate === 0 && sentToday > 0)
      alerts.push('Taux de réponse = 0%');
    if (complianceToday < complianceYesterday - 20)
      alerts.push('Chute importante de compliance');
    if (missedToday > 3)
      alerts.push(`${missedToday} rappels ignorés aujourd'hui`);
  ...
  async getServiceIntelligence() {
    ...
    const [patients, doctors, nurses] = await Promise.all([
      ...
      doctorRole
        ? this.userModel
            .find({ role: doctorRole._id, isArchived: { $ne: true } })
            .lean()
        : [],
      nurseRole
        ? this.userModel
            .find({ role: nurseRole._id, isArchived: { $ne: true } })
            .lean()
        : [],
    ]);
    ...
    const [vitalsToday, symptomsToday, vitals7d, symptoms7d, reminders7d] =
      await Promise.all([
        this.vitalModel
          .find({
            patientId: { $in: patientIds },
        ...
        this.vitalModel
          .find({
            patientId: { $in: patientIds },
            recordedAt: { $gte: since7d },
          })
          .lean(),
        this.symptomModel
          .find({
            patientId: { $in: patientIds },
            reportedAt: { $gte: since7d },
          })
          .lean(),
    ...
    const serviceMap: Record<
      string,
      { patients: number; alerts: number; name: string }
    > = {};
    (patients as any[]).forEach((p) => {
      const dept = p.department?.trim();
      const sid =
        p.serviceId?.toString() ||
        p.assignedService?.toString() ||
        dept ||
        'general';
    ...
    (symptoms7d as any[]).forEach((s) => {
      const p = (patients as any[]).find(
        (pt) => pt._id.toString() === s.patientId?.toString(),
      );
      if (p) {
    ...
    const avgPatientsPerService =
      totalPatients / Math.max(Object.keys(serviceMap).length, 1);
    const overloadedServices = Object.entries(serviceMap)
      .filter(
        ([, v]) => v.patients > avgPatientsPerService * 1.4 || v.alerts > 5,
      )
      .map(([id, v]) => ({
        serviceId: id,
        name: v.name,
        patients: v.patients,
        alerts: v.alerts,
        overloadScore: Math.round(
          (v.patients / Math.max(avgPatientsPerService, 1)) * 50 +
            (v.alerts / 10) * 50,
        ),
      }))
    ...
    const vitalTodaySet = new Set(
      (vitalsToday as any[]).map((v: any) => v.patientId?.toString()),
    );
    const symptomTodaySet = new Set(
      (symptomsToday as any[]).map((s: any) => s.patientId?.toString()),
    );
    const sentReminders = (reminders7d as any[]).filter(
      (r) => r.status === 'sent',
    ).length;
    const respondedCount = (patients as any[]).filter(
      (p) =>
        vitalTodaySet.has(p._id.toString()) &&
        symptomTodaySet.has(p._id.toString()),
    ).length;

    const complianceRate = totalPatients
      ? Math.round((respondedCount / totalPatients) * 100)
      : 0;
    const responseRate = sentReminders
      ? Math.round((respondedCount / sentReminders) * 100)
      : 0;
    ...
    const serviceScores = Object.entries(serviceMap)
      .map(([id, v]) => {
        ...
        const svcCompliant = svcPatients.filter(
          (p) =>
            vitalTodaySet.has(p._id.toString()) &&
            symptomTodaySet.has(p._id.toString()),
        ).length;
        const score =
          svcPatients.length > 0
            ? Math.round(
                (svcCompliant / svcPatients.length) * 70 +
                  (v.alerts < 3 ? 30 : v.alerts < 6 ? 15 : 0),
              )
            : 50;
        return {
          name: v.name,
    ...
    const dailySymptoms = days.map(
      (day) =>
        (symptoms7d as any[]).filter(
          (s) =>
            s.reportedAt?.toISOString?.()?.startsWith(day) ||
            s.reportedAt?.toString?.()?.startsWith(day),
        ).length,
    );
    const trend =
      dailySymptoms.length >= 3
        ? dailySymptoms.slice(-3).reduce((a, b) => a + b, 0) / 3 -
          dailySymptoms.slice(0, 3).reduce((a, b) => a + b, 0) / 3
        : 0;
...
NO TEXT OUTSIDE JSON.`;
    ...
    let summary: any = {
      titre: `Résumé du ${new Date().toLocaleDateString('fr-FR')}`,
      statut:
        criticalSymptoms > 5 || overloadedServices.length > 2
          ? 'CRITIQUE'
          : complianceRate < 30
            ? 'ATTENTION'
            : 'STABLE',
    ...
    return {
      overloadedServices,
      serviceScores: serviceScores.filter(
        (s) => s.name !== 'Général' || serviceScores.length === 1,
      ),
      prediction: {
        risk: predictionRisk,
...
NO TEXT OUTSIDE JSON.`;
  ...
  async generateAuditReport() {
    ...
    const [total, last24h, deletions, criticals] = await Promise.all([
      ...
      this.auditLogModel.countDocuments({
        action: { $in: ['DELETE', 'ARCHIVE'] },
        createdAt: { $gte: since7d },
      }),
      this.auditLogModel.countDocuments({
        action: { $in: ['DELETE', 'ARCHIVE', 'DEACTIVATE', 'RESET_PASSWORD'] },
    ...
    const coordRole = await this.roleModel
    ...
    const activeCoordIds = await this.reminderModel.distinct('sentBy', {
      createdAt: { $gte: since3d },
    });
    const inactiveCoords = (coordinators as any[]).filter(
      (c) =>
        !activeCoordIds
    ...
    const noDataCount = (patients as any[]).filter(
      (p) =>
        !vitalSet.has(p._id.toString()) && !symptomSet.has(p._id.toString()),
    ).length;

    const data = {
      totalAuditLogs: total,
      last24hEvents: last24h,
      deletionsLast7d: deletions,
      criticalActionsLast24h: criticals,
      topActiveUsers: topUsers.map(
        (u: any) => `${u._id ?? 'unknown'} — ${u.count} actions`,
      ),
      inactiveCoordinators: inactiveCoords.length,
      inactiveCoordNames: (inactiveCoords as any[])
        .slice(0, 3)
        .map((c: any) => `${c.firstName} ${c.lastName}`),
      totalPatients: patients.length,
      noDataToday: noDataCount,
    };
      ...
      Math.round(
        (data.noDataToday / Math.max(data.totalPatients, 1)) * 40 +
          (data.inactiveCoordinators /
            Math.max((coordinators as any[]).length, 1)) *
            40 +
...
{"riskScore":${riskScore},"riskLevel":"${riskLevel}","resume":"...","alertes":["🚨 ..."],"risques":["⚠️ ..."],"interpretation":"...","actions":["✅ ..."],"topUsers":["..."]}`;
    ...
    try {
      ...
      return {
        report: { ...report, riskScore, riskLevel },
        data,
        generatedAt: new Date().toISOString(),
      };
    } catch (err: any) {
  ...
  async generateDailyAuditReport() {
    ...
    const [total24h, byAction, byUser, criticals, logins, deletions] =
      await Promise.all([
        this.auditLogModel.countDocuments({ createdAt: { $gte: since24h } }),
        ...
        this.auditLogModel.countDocuments({
          createdAt: { $gte: since24h },
          action: {
            $in: ['DELETE', 'ARCHIVE', 'DEACTIVATE', 'RESET_PASSWORD'],
          },
        }),
        this.auditLogModel.countDocuments({
          createdAt: { $gte: since24h },
    ...
    const patients = patientRole
      ? await this.userModel
          .find({ role: (patientRole as any)._id, isArchived: { $ne: true } })
          .lean()
      : [];
    const patientIds = (patients as any[]).map((p) => p._id);
    const [vitalsToday, symptomsToday] = await Promise.all([
    ...
    const compliantToday = (patients as any[]).filter(
      (p) => vitalSet.has(p._id.toString()) && symptomSet.has(p._id.toString()),
    ).length;
    const complianceRate = patients.length
      ? Math.round((compliantToday / patients.length) * 100)
      : 0;
    ...
    const data = {
      ...
      logins,
      deletions,
      topUsers: (byUser as any[]).map(
        (u: any) => `${u._id} (${u.count} actions)`,
      ),
      actionBreakdown: (byAction as any[])
        .map((a: any) => `${a._id}: ${a.count}`)
        .join(', '),
      totalPatients: patients.length,
      compliantToday,
      complianceRate,
...
{"title":"Rapport d'Audit Journalier — ${data.date}","summary":"...","highlights":["..."],"concerns":["..."],"recommendations":["..."],"complianceRate":${data.complianceRate},"criticalCount":${data.criticalActions},"totalEvents":${data.totalEvents}}`;
    ...
    try {
      ...
      return {
        type: 'daily',
        report: {
          title: `Rapport Journalier — ${data.date}`,
          summary: `${data.totalEvents} événements, ${data.criticalActions} critiques, compliance ${data.complianceRate}%.`,
          highlights: [],
          concerns: [],
          recommendations: [],
          complianceRate: data.complianceRate,
          criticalCount: data.criticalActions,
  ...
  async generateMonthlyComplianceReport() {
    ...
    const coordinators = coordRole
      ? await this.userModel
          .find({ role: (coordRole as any)._id, isArchived: { $ne: true } })
          .lean()
      : [];
    const patientIds = (patients as any[]).map((p) => p._id);

    const [vitals30d, symptoms30d, reminders30d, auditEvents] =
      await Promise.all([
        this.vitalModel
          .find({
            patientId: { $in: patientIds },
    ...
    const vitalPatients = new Set(
      (vitals30d as any[]).map((v: any) => v.patientId?.toString()),
    );
    const symptomPatients = new Set(
      (symptoms30d as any[]).map((s: any) => s.patientId?.toString()),
    );
    const activePatients = (patients as any[]).filter(
      (p) =>
        vitalPatients.has(p._id.toString()) ||
        symptomPatients.has(p._id.toString()),
    ).length;
    const sentReminders = (reminders30d as any[]).filter(
      (r: any) => r.status === 'sent',
    ).length;
    const complianceRate = patients.length
    ...
    const data = {
      ...
      vitalsSubmitted: vitals30d.length,
      symptomsSubmitted: symptoms30d.length,
      remindersSent: sentReminders,
      totalReminders: reminders30d.length,
      auditEvents,
    };
...
{"title":"Rapport de Compliance Mensuel — ${data.month}","executiveSummary":"...","complianceAnalysis":"...","strengths":["..."],"weaknesses":["..."],"recommendations":["..."],"kpis":{"complianceRate":${data.complianceRate},"activePatients":${data.activePatients},"remindersSent":${data.remindersSent}}}`;
    ...
    try {
      ...
      return {
        type: 'monthly',
        report: {
          title: `Rapport Mensuel — ${data.month}`,
          executiveSummary: `Compliance ${data.complianceRate}% sur ${data.totalPatients} patients.`,
          complianceAnalysis: '',
          strengths: [],
          weaknesses: [],
          recommendations: [],
  ...
  async generateSuspiciousActivityReport() {
    ...
    const data = {
      ...
      multiIpUsers: (multiIpUsers as any[]).map(
        (u: any) => `${u._id} (${u.ips.length} IPs, ${u.count} actions)`,
      ),
      massDeleteUsers: (massDeletes as any[]).map(
        (u: any) => `${u._id} (${u.count} suppressions)`,
      ),
      topSuspicious: (suspiciousLogs as any[])
        .slice(0, 5)
        .map((l: any) => `${l.userEmail} — ${l.action} sur ${l.entityType}`),
    };
...
{"title":"Rapport d'Activités Suspectes — ${data.period}","riskLevel":"${data.suspiciousCount > 10 ? 'HIGH' : data.suspiciousCount > 3 ? 'MEDIUM' : 'LOW'}","summary":"...","threats":["..."],"suspiciousUsers":["..."],"recommendations":["..."],"immediateActions":["..."]}`;
    ...
    try {
      ...
      return {
        type: 'suspicious',
        report: {
          title: `Rapport Activités Suspectes — ${data.period}`,
          riskLevel: data.suspiciousCount > 10 ? 'HIGH' : 'LOW',
          summary: `${data.suspiciousCount} événements suspects détectés.`,
          threats: [],
          suspiciousUsers: [],
          recommendations: [],
          immediateActions: [],
  ...
  private encodeSmokingStatus(smoke: string | undefined): number {
    ...
    if (s.includes('smok') && !s.includes('never') && !s.includes('former'))
      return 2;
  ...
  private buildRecommendations(riskLevel: string, input: any): string[] {
    ...
    if (input.hypertension)
      recs.push('Traitement antihypertenseur à surveiller');
    if (input.heart_disease)
      recs.push('Suivi cardiologique régulier obligatoire');
    if (input.avg_glucose_level > 150)
      recs.push('Glycémie élevée — consulter un endocrinologue');
    if (input.bmi > 30)
      recs.push('IMC élevé — programme de perte de poids conseillé');
  ...
  async predictStrokeRisk(patientId: string) {
    ...
    const systolic =
      ...
      0;
    const hypertension =
      nurseDossier?.medicalHistory?.hypertension || systolic > 140 ? 1 : 0;

    // ── HEART DISEASE ────────────────────────────────────────
    const heart_disease =
      nurseDossier?.medicalHistory?.heartDisease ||
      (latestVital?.heartRate && latestVital.heartRate > 100)
        ? 1
        : 0;
    ...
    try {
      ...
      this.logger.log(
        `✅ ML result: level=${flaskPrediction.riskLevel}, score=${flaskPrediction.riskScore}`,
      );

      // 🔥 Utiliser les recommandations de Flask (plus complètes) OU générer les nôtres
      const recommendations =
        flaskRecommendations.length > 0
      ...
      this.logger.error(
        `❌ ML prediction failed for ${patientId}: ${err.message}`,
      );
      ...
      return {
        ...
        prediction: {
          ...
          recommendations: [
            'Service ML indisponible — Veuillez réessayer plus tard',
          ],
        },
  ...
  async predictAllPatientsRisk(doctorId?: string) {
    ...
    const query: any = {
      role: (patientRole as any)._id,
      isArchived: { $ne: true },
    };

    if (doctorId) {
      query.$or = [{ doctorId: doctorId }, { assignedDoctor: doctorId }];
    }
    ...
    const results = await Promise.all(
      (patients as any[]).map((p) =>
        this.predictStrokeRisk(p._id.toString()).catch(() => null),
      ),
    );
    ...
    return results
      ...
      .sort(
        (a: any, b: any) => b.prediction.riskScore - a.prediction.riskScore,
      );
  }
}
